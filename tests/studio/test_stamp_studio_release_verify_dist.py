...
def _write_sdist(path: Path, version: str) -> None:
    with tarfile.open(path, "w:gz") as archive:
        ...
        member = tarfile.TarInfo(name = f"unsloth-0.0.0/{stamp.BUILD_INFO_SUFFIX}")
...
def test_verify_dist_passes_without_expected_when_stamp_is_valid_and_consistent():
    with tempfile.TemporaryDirectory() as directory:
        ...
        _write_wheel(dist_dir / "unsloth-0.0.0-py3-none-any.whl", "v0.1.40-beta")
        _write_sdist(dist_dir / "unsloth-0.0.0.tar.gz", "v0.1.40-beta")
...
def test_verify_dist_requires_consistent_stamp_across_artifacts():
    with tempfile.TemporaryDirectory() as directory:
        dist_dir = Path(directory)
        _write_wheel(dist_dir / "unsloth-0.0.0-py3-none-any.whl", "v0.1.40-beta")
        _write_sdist(dist_dir / "unsloth-0.0.0.tar.gz", "v0.1.41-beta")
